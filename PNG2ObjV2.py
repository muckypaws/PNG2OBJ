...
def main():
    ...
    # Create Background Primitive - Usually Colour Code #000000
   # bground = create_primitive(0, 0, pattern_w, pattern_h, cube_vertices, joint_faces, 0, 0)
    ...
    resp = processFile(0,fileCounter)
    if resp == False:
        exit(0)

    print(f"Found : {len(mtl_colour_dict)-1} Colours")

    # Delete Background Colour of 000000
    # del mtl_colour_dict["#000000"]

    for pixelCount in sorted(mtl_colour_dict.items(), key=lambda kv: kv[1], reverse=True):
        print(f"{pixelCount}")

    for colourIndex in range(len(mtl_colour_dict)):
        resp = processFile(colourIndex,fileCounter)
        fileCounter+=1
...
def processFile(colourIndex, fileCounter):
    CUBE_X = 10
    CUBE_Y = -10
    ...
    obj_file = os.path.join(PATTERNS, "{}{}.obj".format(sys.argv[1],str(fileCounter)))
    txt_file = os.path.join(PATTERNS, "{}{}.txt".format(sys.argv[1],str(fileCounter)))
    mtl_filename = os.path.join(PATTERNS, "{}{}.mtl".format(sys.argv[1],str(fileCounter)))
